...
export function ProjectWorkspace({ projectId, initialState }: ProjectWorkspaceProps) {
  ...
  // Toast notifications for planning progress
  ...
  useEffect(() => {
    ...
    // Get the most recent planning event
    const planningEvents = events.filter(e => e.stage === "planning")
    if (planningEvents.length === 0) return

    const latestEvent = planningEvents[planningEvents.length - 1]

    // Only show toast if this is a new event we haven't seen yet
    if (latestEvent.id !== lastSeenEventId.current) {
      lastSeenEventId.current = latestEvent.id

      // Show toast based on event level
      if (latestEvent.level === "error") {
        toast.error(latestEvent.message, {
          duration: 5000,
        })
      } else if (latestEvent.level === "warn") {
        toast.warning(latestEvent.message, {
          duration: 4000,
        })
      } else {
