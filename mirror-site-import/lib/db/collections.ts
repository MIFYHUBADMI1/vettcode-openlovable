...
export async function ensureIndexes() {
  ...
  await Promise.all([
    ...
    // If index creation races or the DB is briefly unavailable, don't crash
    // the request — the next call will retry since `indexesEnsured` should
    // only guard against duplicate *successful* setup. Reset on failure.
