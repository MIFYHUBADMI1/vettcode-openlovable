 ...
 */
export async function execute<Body, T>(
  config: ResolvedConfig,
  args: ExecuteArgs<Body>,
): Promise<{ envelope: Extract<RuntimeEnvelopeResult<T>, { ok: true }>; requestId?: string }> {
  ...
  try {
    ...
    if (e instanceof AtaiError) throw e
 ...
 * result/error model.
 */
async function handleResponse<T>(response: Response): Promise<{ envelope: Extract<RuntimeEnvelopeResult<T>, { ok: true }>; requestId?: string }> {
 ...
 */
function httpErrorFromResponse(status: number, rawText: string | null, requestId?: string, retryAfterMs?: number): AtaiError {
...
function isErrorEnvelope(parsed: unknown): parsed is { ok: false; error: { code: string; message: string } } {
...
function isSuccessEnvelope<T>(parsed: unknown): parsed is Extract<RuntimeEnvelopeResult<T>, { ok: true }> {
  if (typeof parsed !== "object" || parsed === null) return false
  const obj = parsed as Record<string, unknown>
