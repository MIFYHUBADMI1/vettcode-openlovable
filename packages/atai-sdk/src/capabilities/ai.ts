 ...
 */
function validateChatInput(input: AiChatInput): void {
...
export class AiCapability {
   ...
   */
  async chat(input: AiChatInput, options?: AtaiRequestOptions): Promise<AiChatResult> {
    ...
    const { envelope } = await execute<typeof body, AiChatResult>(this.config, {
      path: "",
      method: "POST",
      body,
      options,
 ...
 */
function assertChatResult(payload: unknown): AiChatResult {
  ...
  const valid =
    ...
    (p.usage === undefined ||
      (typeof p.usage === "object" &&
        p.usage !== null &&
        typeof (p.usage as Record<string, unknown>).promptTokens === "number" &&
        typeof (p.usage as Record<string, unknown>).completionTokens === "number" &&
        typeof (p.usage as Record<string, unknown>).totalTokens === "number"))
  if (!valid) {
    throw new AtaiError("atai_invalid_response", "The Atai Runtime API returned an unexpected response shape.")
  }
  return payload as unknown as AiChatResult
}
