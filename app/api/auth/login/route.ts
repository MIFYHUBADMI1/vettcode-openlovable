...
export async function POST(req: Request) {
  try {
    ...
    if (user.banned) {
      return fail("FORBIDDEN", "This account has been banned. Please contact support.", 403)
    }
    if (user.suspended) {
      return fail("FORBIDDEN", "This account has been suspended. Please contact support.", 403)
    }
