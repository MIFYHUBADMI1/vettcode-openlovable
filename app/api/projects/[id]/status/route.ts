 ...
 * or users poll the same project simultaneously.
 */
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Use single-flight to deduplicate concurrent status checks for the same project
    const fetcher = singleFlight<Response>(`api.projects.status:${id}`)
      ...
      try {
        ...
        if (project.state === "deploying") {
          ...
          if (deployStatus.status === "success") {
            ...
            const screenshotTarget = productionUrl || project.developmentUrl
            if (screenshotTarget) {
              void captureAppPreview(id, screenshotTarget)
