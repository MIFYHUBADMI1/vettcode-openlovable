...
import { getFullConversation, isTotalumConfigured } from "@/lib/integrations/totalum/service"
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const conversation = await getFullConversation(project.totalumProjectId)
    return ok(conversation)
  } catch (e) {
    return handleRouteError("api.projects.agent.full-conversation", e)
  }
}
