...
export async function POST(req: Request) {
  try {
    const user = await requireUser()
    const body = (await req.json().catch(() => ({}))) as { mode?: string; url?: string; idea?: string }
    ...
    if (mode === "website") {
      ...
      const project = newProject(user.id, { mode, sourceUrl, name: new URL(sourceUrl).host })
    ...
    const project = newProject(user.id, { mode, idea, name: "New app" })
    await store.createProject(project)
