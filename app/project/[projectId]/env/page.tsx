      ...
      <main className="min-h-svh bg-background text-foreground">
        ...
        <div className="mx-auto max-w-3xl px-6 py-10">
          <Link
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground transition-all hover:border-primary/30 hover:bg-accent hover:text-foreground"
          >
            ← Back to workspace
          </Link>
          <div className="mt-16 flex flex-col items-center gap-4 text-center">
            <EnvPageIcons.NotBuiltIcon />
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-10">
        <div className="mb-8 flex flex-col gap-4 border-b border-border pb-8">
          ...
          <div className="flex items-start gap-3">
            <EnvPageIcons.HeaderIcon />
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Environment Variables</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                Manage <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">.env</code> secrets for{" "}
                <span className="font-medium text-foreground">{project.name}</span>. Values are encrypted and injected at runtime — they are never returned after saving.
              </p>
            </div>
          </div>
        </div>
