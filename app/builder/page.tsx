...
export default function HomePage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        ...
        setUserModels(parsed);
        // Set first selected model as default if current default not in selection
        if (parsed.length > 0 && !parsed.includes(selectedModel)) {
          setSelectedModel(parsed[0]);
        }
      } else {
        // Use default free models
  ...
  const models = userModels.map(model => ({
    id: model,
                ...
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
                  ...
                  <div className="px-[28px] pt-0 pb-[28px]">
                    <div className="border-t border-gray-100 bg-white">
                      ...
                      <div className={`flex flex-col gap-2 mt-2 pb-4 transition-all duration-300 transform ${
                        isValidUrl ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0'
                        ...
                        <p className="text-xs text-gray-500">
                          Showing {userModels.length}/3 models
                        </p>
                      </div>
                    </div>
                  </div>
